...
async function buildBundle(options: BuildOptions = {}) {
  const { outputName = "nexus-ux", excludeModules = [], gitRef, minify = false, appDir, twVersion = "latest" } = options;
  ...
  try {
    ...
    if (appDir) {
      ...
      try {
        const cdnBase = `https://cdn.jsdelivr.net/npm/tailwindcss@${twVersion}`;
        const fetchCss = async (file: string) => {
          const res = await fetch(`${cdnBase}/${file}`);
          if (!res.ok) throw new Error(`Failed to fetch ${file} from ${cdnBase}`);
          return await res.text();
        };

        const [indexCss, preflightCss, themeCss, utilitiesCss] = await Promise.all([
          fetchCss("index.css"),
          fetchCss("preflight.css"),
          fetchCss("theme.css"),
          fetchCss("utilities.css")
        ]);
