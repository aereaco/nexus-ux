// ../sortable/package.json
var version = "1.15.7";

// ../sortable/src/BrowserInfo.js
...
function getChild(el, childNum, options, includeDragEl) {
  ...
  while (i < children.length) {
    if (children[i].style.display !== "none" && children[i] !== Sortable_default.ghost && (includeDragEl || children[i] !== Sortable_default.dragged) && closest(children[i], options.draggable, el, false)) {
...
function lastChild(el, selector) {
  ...
  while (last && (last === Sortable_default.ghost || css(last, "display") === "none" || selector && !matches(last, selector))) {
...
function index(el, selector) {
  ...
  while (el = el.previousElementSibling) {
    if (el.nodeName.toUpperCase() !== "TEMPLATE" && el !== Sortable_default.clone && (!selector || matches(el, selector))) {
      index2++;
    }
  }
  return index2;
  ...
  return {
    captureAnimationState() {
      ...
      children.forEach((child) => {
        if (css(child, "display") === "none" || child === Sortable_default.ghost) return;
    ...
    animateAll(callback) {
      ...
      animationStates.forEach((state) => {
        let time = 0, animatingThis = false, target = state.target, fromRect = target.fromRect, toRect = getRect(target), prevFromRect = target.prevFromRect, prevToRect = target.prevToRect, animatingRect = state.rect, targetMatrix = matrix(target, true);
        if (targetMatrix) {
...
function dispatchEvent({
  ...
  let allEventProperties = { ...extraEventProperties, ...PluginManager_default.getEventProperties(name, sortable) };
...
var pluginEvent = function(eventName, sortable, { evt: originalEvent, ...data } = {}) {
  PluginManager_default.pluginEvent.bind(Sortable)(eventName, sortable, {
    dragEl,
    parentEl,
    ghostEl,
    rootEl,
...
var _dragElInRowColumn = function(dragRect, targetRect, vertical) {
...
var _prepareGroup = function(options) {
  function toFn(value, pull) {
    return function(to, from, dragEl3, evt) {
      ...
      if (value == null && (pull || sameGroup)) {
        ...
        return toFn(value(to, from, dragEl3, evt), pull)(to, from, dragEl3, evt);
      } else {
...
function Sortable(el, options) {
  ...
  let defaults2 = {
    ...
    animation: 0,
    easing: null,
    setData: function(dataTransfer, dragEl3) {
      dataTransfer.setData("Text", dragEl3.textContent);
    },
    dropBubble: false,
  ...
  PluginManager_default.initializePlugins(this, el, defaults2);
...
Sortable.prototype = /** @lends Sortable.prototype */
{
  ...
  option: function(name, value) {
    ...
    if (value === void 0) {
      ...
      let modifiedValue = PluginManager_default.modifyOption(this, name, value);
...
function onMove(fromEl, toEl, dragEl3, dragRect, targetEl, targetRect, originalEvent, willInsertAfter) {
...
Sortable.mount = function(...plugins2) {
  ...
  plugins2.forEach((plugin) => {
    ...
    PluginManager_default.mount(plugin);
  });
};
...
function MultiDragPlugin() {
  ...
  MultiDrag.prototype = {
    ...
    delayEnded() {
      this.isMultiDrag = ~multiDragElements.indexOf(dragEl2);
    },
    setupClone({ sortable, cancel }) {
      ...
      for (let i = 0; i < multiDragElements.length; i++) {
        ...
        multiDragElements[i] === dragEl2 && toggleClass(multiDragClones[i], this.options.chosenClass, false);
      }
      sortable._hideClone();
      cancel();
    },
    ...
    dragStarted({ sortable }) {
      ...
      if (this.options.sort) {
        ...
        if (this.options.animation) {
          multiDragElements.forEach((multiDragElement) => {
            if (multiDragElement === dragEl2) return;
            css(multiDragElement, "position", "absolute");
          });
          let dragRect = getRect(dragEl2, false, true, true);
          multiDragElements.forEach((multiDragElement) => {
            if (multiDragElement === dragEl2) return;
    ...
    dragOverCompleted({ sortable, isOwner, insertion, activeSortable, parentEl: parentEl2, putSortable: putSortable2 }) {
      ...
      if (insertion) {
        ...
        if (options.animation && multiDragElements.length > 1 && (folding || !isOwner && !activeSortable.options.sort && !putSortable2)) {
          let dragRectAbsolute = getRect(dragEl2, false, true, true);
          multiDragElements.forEach((multiDragElement) => {
            if (multiDragElement === dragEl2) return;
    ...
    dragOverAnimationCapture({ dragRect, isOwner, activeSortable }) {
      ...
      if (activeSortable.options.animation && !isOwner && activeSortable.multiDrag.isMultiDrag) {
        ...
        let dragMatrix = matrix(dragEl2, true);
    ...
    drop({ originalEvent: evt, rootEl: rootEl2, parentEl: parentEl2, sortable, dispatchSortableEvent, oldIndex: oldIndex2, putSortable: putSortable2 }) {
      ...
      if (!dragStarted) {
        ...
        toggleClass(dragEl2, options.selectedClass, !~multiDragElements.indexOf(dragEl2));
        if (!~multiDragElements.indexOf(dragEl2)) {
          multiDragElements.push(dragEl2);
          dispatchEvent({
            sortable,
            rootEl: rootEl2,
            name: "select",
          ...
          if (evt.shiftKey && lastMultiDragSelect && sortable.el.contains(lastMultiDragSelect)) {
            let lastIndex = index(lastMultiDragSelect), currentIndex = index(dragEl2);
            ...
            lastMultiDragSelect = dragEl2;
          }
          multiDragSortable = toSortable;
        } else {
          multiDragElements.splice(multiDragElements.indexOf(dragEl2), 1);
      ...
      if (dragStarted && this.isMultiDrag) {
        ...
        if ((parentEl2[expando].options.sort || parentEl2 !== rootEl2) && multiDragElements.length > 1) {
          let dragRect = getRect(dragEl2), multiDragIndex = index(dragEl2, ":not(." + this.options.selectedClass + ")");
          if (!initialFolding && options.animation) dragEl2.thisAnimationDuration = null;
          toSortable.captureAnimationState();
          if (!initialFolding) {
            if (options.animation) {
              dragEl2.fromRect = dragRect;
              multiDragElements.forEach((multiDragElement) => {
                multiDragElement.thisAnimationDuration = null;
                if (multiDragElement !== dragEl2) {
  ...
  return Object.assign(MultiDrag, {
    ...
    eventProperties() {
      ...
      multiDragElements.forEach((multiDragElement) => {
        ...
        let newIndex2;
        if (folding && multiDragElement !== dragEl2) {
          newIndex2 = -1;
        } else if (folding) {
...
var MultiDrag_default = MultiDragPlugin;
...
function SwapPlugin() {
  ...
  Swap.prototype = {
    ...
    drop({ activeSortable, putSortable: putSortable2, dragEl: dragEl3 }) {
      let toSortable = putSortable2 || this.sortable;
      let options = this.options;
      lastSwapEl && toggleClass(lastSwapEl, options.swapClass, false);
      if (lastSwapEl && (options.swap || putSortable2 && putSortable2.options.swap)) {
        if (dragEl3 !== lastSwapEl) {
          toSortable.captureAnimationState();
          if (toSortable !== activeSortable) activeSortable.captureAnimationState();
          swapNodes(dragEl3, lastSwapEl);
          toSortable.animateAll();
...
var Swap_default = SwapPlugin;

// bundle-entry.js
Sortable_default.mount(new MultiDrag_default());
Sortable_default.mount(new Swap_default());
export {
  Sortable_default as Sortable
};
/**!
 * Sortable
