...
directive('route', (el: any, { value, expression, modifiers }: any, { evaluate }: any) => {
    ...
    const beforeLeave = el.getAttribute('data-route:before-leave')
    if (beforeLeave) routeDef.beforeLeave = beforeLeave

    const afterLeave = el.getAttribute('data-route:after-leave')
    if (afterLeave) routeDef.afterLeave = afterLeave
