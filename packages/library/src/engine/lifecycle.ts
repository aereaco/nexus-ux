...
export function start() {
    if (started) warn('State has already been initialized on this page. Calling State.start() more than once can cause problems.')

    started = true

    if (! document.body) warn('Unable to initialize. Trying to load State before `<body>` is available. Did you forget to add `defer` in State\'s `<script>` tag?')

    dispatch(document, 'state:init')
    dispatch(document, 'state:initializing')
