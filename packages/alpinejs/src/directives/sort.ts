...
export default function (Alpine: any) {
    Alpine.directive('sort', (el: any, { value, modifiers, expression }: any, { effect, evaluate, evaluateLater, cleanup }: any) => {
        if (value === 'config') {
...
function generateSortHandler(expression: any, evaluateLater: any) {
    ...
    const AlpineLocal = (window as any).Alpine

    AlpineLocal && AlpineLocal.dontAutoEvaluateFunctions(() => {
            handle(
