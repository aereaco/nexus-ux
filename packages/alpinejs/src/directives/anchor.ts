...
export default function (Alpine: any) {
    Alpine.magic('anchor', (el: any) => {
        if (! el._data_anchor) throw 'Alpine: No data-anchor directive found on element using $anchor...'
    ...
    Alpine.directive('anchor', Alpine.skipDuringClone((el: any, { expression, modifiers, value }: any, { cleanup, evaluate }: any) => {
        let { placement, offsetValue, unstyled } = getOptions(modifiers)

        el._data_anchor = Alpine.reactive({ x: 0, y: 0 })

        let reference = evaluate(expression)

        if (! reference) throw 'Alpine: no element provided to data-anchor...'
