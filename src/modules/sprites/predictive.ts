...
class PredictiveEngine {
      ...
      const setup = () => {
        if (
          ...
          line.style.transition =
            "x2 0.1s linear, y2 0.1s linear, opacity 0.3s ease-out";
          ...
          targetLine.style.transition =
            "x2 0.1s linear, y2 0.1s linear, opacity 0.1s ease";
  ...
  private getDynamicSelectors(): string {
    ...
    if (typeof document === "undefined") return Array.from(baseSelectors).join(", ");
            ...
            for (let j = 0; j < rules.length; j++) {
              ...
              if (
                ...
                for (let segment of segments) {
                  ...
                  const clean = segment.replace(/:hover|:active|:focus/g, "")
                    .trim();
...
// Expose quadtree globally for ZCZS O(log n) spatial queries
(globalThis as any)._nexusQuadtree = predictive.quadtree;
