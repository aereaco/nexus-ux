...
// ============================================================================
// Only framework-specific constants (PACKED_COMPONENTS, PACKED_KEYFRAMES) are
// bundled. Tailwind preflight and theme are fetched at runtime from CDN to
// avoid embedding ~22KB of CSS strings in the bundle.
import {
  PACKED_COMPONENTS,
  PACKED_KEYFRAMES,
} from '../manifest.ts';
...
class StyleSheetManager {
  ...
  adoptClass(className: string, el?: HTMLElement, runtime?: RuntimeContext): void {
    ...
    try {
      ...
      // Cache the class only if we resolved rules (or if JIT engine is not fully booted, we don't cache failed ones yet)
      if (compiledAny || !_isJitEngineBooted) {
...
export function populateStandardVariants(ds: DesignSystem) {
  ...
  pseudos.forEach(p => ds.variant(p, (_c) => []));
...
if (typeof document !== 'undefined' && typeof CSSStyleSheet !== 'undefined') {
  ...
  (async () => {
    try {
      ...
      // CDN unreachable — preflight/theme will be absent, JIT engine still works.
    }
  })();
}
