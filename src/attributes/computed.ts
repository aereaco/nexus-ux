...
const computedModule: AttributeModule = {
  ...
  handle: (el: HTMLElement, value: string, runtime: RuntimeContext): (() => void) | void => {
    ...
    if (el.hasAttribute('data-computed')) {
      // const value = el.getAttribute('data-computed'); // Use argument instead
      try {
        ...
        if (typeof computedDefs === 'object' && computedDefs !== null) {
          Object.entries(computedDefs).forEach(([propName, getter]) => {
             ...
             try {
                const computedVal = runtime.computed(getter as () => unknown);
               ...
               initError('computed', `Failed to init computed ${propName}`, el, String(getter));
             }
          });
        }
      } catch (_e) {
        initError('computed', `Failed to parse data-computed JSON`, el, value || '');
      }
    }
